...
export default function MainPage({ onLogout }) {
  ...
  return (
    <div className="h-screen w-full bg-[#070707] text-white uppercase overflow-hidden flex flex-col font-sans relative">
      ...
      {/* UNIQUE MUSIC ANIMATION: SONIC SCAN WIPE */}
      <motion.div 
        initial={{ y: "-100%" }}
        animate={{ y: "100vh" }}
        transition={{ duration: 1.5, ease: "easeInOut" }}
        className="fixed inset-x-0 h-[15vh] bg-gradient-to-b from-transparent via-yellow-400/20 to-transparent z-[100] pointer-events-none"
      >
        <div className="h-[2px] w-full bg-yellow-400 shadow-[0_0_30px_#fbbf24]" />
      </motion.div>
      ...
      >
        ...
        <nav className="flex justify-between items-center px-10 h-24 text-[10px] tracking-[0.4em] z-20 shrink-0 border-b border-white/5">
          ...
          <div className="hidden md:flex gap-12 opacity-40">
            <a href="#" className="hover:opacity-100 transition">Extractor</a>
            <button onClick={() => setIsModalOpen(true)} className="hover:text-red-500 transition">Logout</button>
          </div>
        ...
        <main className="flex-1 flex flex-col lg:flex-row min-h-0 w-full">
          ...
          <div className="flex-1 flex flex-col justify-between p-10 lg:p-16 overflow-hidden">
            <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 6rem)' }} className="leading-[0.85] font-['Archivo_Black'] tracking-tighter">DRUM NOTE <br /><span className="text-yellow-400">EXTRACTOR</span></h1>
            <div className="space-y-12">
              <div className="border border-dashed border-white/20 p-8 text-center hover:border-yellow-400 transition-colors group relative bg-white/[0.02]">
                ...
                <div className="text-[10px] tracking-[0.3em] opacity-60 group-hover:opacity-100">{file ? `TRACK: ${file.name}` : "UPLOAD SOURCE"}</div>
              </div>
              <div className="flex flex-row justify-between items-end">
                <div className="text-[8px] opacity-30 tracking-[0.2em] font-medium"><p>MODEL: NEURAL-TRANSFORMER</p><p>STATUS: SYSTEM READY</p></div>
